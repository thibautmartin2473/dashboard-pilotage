...
3. Le script upsert dans `calendar_events` et `mail_items` (RLS sans policy anon, expéditeur/sujet/date seulement) puis supprime les lignes absentes du fichier : ce sont des caches. Seuls les événements `origin = 'google'` sont purgés (ceux créés sur le site, `origin = 'local'`, jamais) ; 50 mails au plus. Chaque mail porte `source` (`gmail` ou `edhec`) et `unread`. Règle EDHEC : libellé Gmail EDHEC, ou expéditeur / destinataire en `edhec.com` ; sinon `gmail`. `colorId` Google (facultatif) va dans `calendar_events.color_id` (`supabase/agenda-links.sql`) : l'accueil en tire trois couleurs — rouge Tomate (11) = plage/cours EDHEC, bleu Myrtille (9) ou absent = autre plage, orange Mandarine (6) = tâche placée côté Google dans l'agenda (`lib/home.js`, `eventColor`). Un conflit (`buildWeek().conflicts`, tuile « À trancher ») n'oppose que deux plages (non-tâche) ou deux tâches Google (toutes deux `color_id = 6`) qui se chevauchent : une tâche orange posée sur une plage rouge/bleue est volontaire, jamais un conflit.
