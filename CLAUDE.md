...
4. Sécurité : `proxy()` (`proxy.js`) laisse passer `/api/mcp` et `/api/mcp/...` sans Basic Auth (claude.ai ne sait pas s'y authentifier ; test de chemin exact, pas un préfixe dans le matcher : `/api/mcp-foo` reste protégé), mais toute requête exige `MCP_SECRET` (≥ 32 caractères, uniquement `[A-Za-z0-9_-]`, générés au hasard) dans le chemin ou en `Authorization: Bearer`, comparé à temps constant ; variable absente ou trop courte = 401 partout. Limite connue : le chemin figure dans les journaux de requêtes de Vercel (le code ne le journalise jamais) ; en cas de fuite, changer `MCP_SECRET` et redéclarer le connecteur. Les erreurs Supabase ne sont jamais renvoyées au client (message générique, détail dans les journaux du serveur). Le texte des saves est du contenu tiers : les sorties le rappellent au modèle.
