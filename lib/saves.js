...
const LIST_COLS = 'url,caption,author,saved_at,tags,category,kind,ville,arrondissement,arrondissements,cuisine,adresse,resume,retenir';
const DETAIL_COLS = `${LIST_COLS},transcript`;
const CANDIDATES = 300;

// Jamais de « liste vide » quand la requête a échoué (bug 127e2db) : l'erreur remonte.
function dbError(error) {
  if (error.code === '42703' || error.code === '42704') {
    return new Error('Schéma des saves obsolète : exécuter supabase/instagram_v2.sql dans Supabase (SQL Editor).');
  }
  if (error.code === 'PGRST205') return new Error('Table instagram_saves absente : exécuter supabase/instagram.sql puis instagram_v2.sql.');
  ...
  return [
    {
      ...
      description:
        ...
        "Lecture seule ; renvoie thème, usage, compte, date d'enregistrement, lieu, adresse, résumé et lien.",
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      name: 'get_save',
      description:
        "Détail complet d'UNE save Instagram de Thibaut à partir de son lien (renvoyé par search_saves) : légende, transcription du reel si elle existe, sujets, lieu. " +
