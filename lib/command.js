...
//
// interpret(texte, now, { events, tasks }) -> { ok: true, actions } | { ok: false, message, examples }
// action = { kind: 'event', title, start, end, label }        (start/end : AAAA-MM-JJTHH:MM, Paris)
//        | { kind: 'task', title, due_date, event_id, label } (« pendant la prochaine session X » : plage auto)
//        | { kind: 'idea', content, task_id, event_id, label } (« pendant la prochaine session X » : cible auto)
...
// ---- Idée : cible automatique (« pendant la prochaine session fit », « pendant le cas »,
// « pendant ma tâche corinne ») ----

const foldSimple = (s) => String(s ?? '').normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase();

const LINK = new RegExp(
  String.raw`\s+(?:` +
    String.raw`(?:pendant|pour|a)\s+la\s+prochaine\s+session\s+` +
    String.raw`|pendant\s+ma\s+(?:prochaine\s+)?tache\s+` +
    String.raw`|pendant\s+(?:le|la|l')\s+` +
    String.raw`)(.+)$`,
  'id'
);

// Sépare le texte d'une idée de sa clause de liaison (mot-clé conservé dans la casse saisie).
// Longueurs de `raw` et de son repli minuscule/sans accent différentes (script rare) : pas de clause.
export function extractIdeaLink(raw) {
  raw = String(raw ?? '');
  const fold = foldSimple(raw);
  if (fold.length !== raw.length) return { content: raw, keyword: null };
  const link = LINK.exec(fold);
  if (!link) return { content: raw, keyword: null };
  const [start, end] = link.indices[1];
  return { content: raw.slice(0, link.index), keyword: raw.slice(start, end).trim() };
}

// Cible d'un mot-clé : le prochain événement à venir (starts_at/ends_at) dont le titre le contient,
// sinon la tâche non faite correspondante la plus proche. `events`/`tasks` : lignes brutes de
// calendar_events / tasks (id, title, starts_at/ends_at, due_date/done_at).
export function matchIdeaTarget(keyword, { events = [], tasks = [], now = new Date() } = {}) {
  const kw = foldSimple(keyword);
  if (!kw) return null;
  const has = (title) => {
    const t = foldSimple(title);
    return t.length > 0 && (t.includes(kw) || (kw.length > 2 && t.length > 2 && kw.includes(t)));
  };
  const soon = [...events]
    .filter((e) => has(e.title) && new Date(e.ends_at ?? e.starts_at) > now)
    .sort((a, b) => new Date(a.starts_at) - new Date(b.starts_at))[0];
  if (soon) return { event_id: soon.id, task_id: null, label: `Agenda : ${soon.title}` };
  const task = [...tasks]
    .filter((t) => !t.done_at && has(t.title))
    .sort((a, b) => (a.due_date ?? '9999-99-99').localeCompare(b.due_date ?? '9999-99-99'))[0];
  if (task) return { event_id: null, task_id: task.id, label: `Tâche : ${task.title}` };
  return null;
}
...
function parse(input, now, data) {
  ...
  if (hit) {
    const { content: cutRaw, keyword } = extractIdeaLink(raw.slice(hit[0].length));
    const content = need(clean(cutRaw, { edges: false }), IDEA_MAX, "le texte de l'idée");
    const target = keyword ? matchIdeaTarget(keyword, { events: data.events, tasks: data.tasks, now }) : null;
    const note = keyword ? (target ? ` · liée à ${target.label}` : ` · aucune cible ne correspond à « ${keyword} »`) : '';
    return [{
      kind: 'idea', content, task_id: target?.task_id ?? null, event_id: target?.event_id ?? null,
      label: `Ajouter l'idée « ${content} »${note}`,
    }];
  }
  ...
  if (hit) {
    ...
    // « pendant la prochaine session fit » (etc.) : place la tâche dans la plage correspondante.
    const { content: cutBody, keyword } = extractIdeaLink(body);
    const title = need(clean(cutBody), TITLE_MAX, 'le titre de la tâche');
    const target = keyword ? matchIdeaTarget(keyword, { events: data.events, tasks: data.tasks, now }) : null;
    const event_id = target?.event_id ?? null;
    const when = due_date ? `pour ${dayLabel(due_date)}` : 'sans échéance';
    const note = keyword ? (event_id ? ` · placée dans ${target.label}` : ` · aucune plage ne correspond à « ${keyword} »`) : '';
    return [{ kind: 'task', title, due_date, event_id, label: `Créer la tâche « ${title} » · ${when}${note}` }];
  }
...
export function interpret(input, now = new Date(), data = {}) {
  try {
    return { ok: true, actions: parse(input, now, data) };
  } catch (err) {
