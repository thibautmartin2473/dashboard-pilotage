...
// Sections d'« Actions à faire » : en retard (échéance passée, quel que soit le
...
export function splitTasks(tasks, today) {
  ...
  for (const t of tasks) {
    ...
    const section = isOverdue(t, today)
      ? 'overdue'
      : t.bucket === 'today' || t.due_date === today
        ? 'today'
        : t.bucket;
...
export const HOME_PANELS = {
  ...
  suggestions: { label: 'Suggestions', wide: false },
  actions: { label: 'Actions à faire', wide: true },
