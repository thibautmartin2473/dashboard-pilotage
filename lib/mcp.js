...
// Erreur due à l'appelant (argument invalide) : son message est montré tel quel au modèle.
...
async function callTool(id, params, tools) {
  ...
  try {
    ...
    return ok(id, { content: [{ type: 'text', text: known ? err.message : `Erreur serveur : ${err.message}` }], isError: true });
  }
}
...
export async function handleMcpRequest(request, { segments, secret, tools }) {
  if (!isAuthorized(request, segments, secret)) {
    if (typeof secret !== 'string' || secret.length < MIN_SECRET_LENGTH) {
      console.error(`[mcp] MCP_SECRET absent ou trop court (< ${MIN_SECRET_LENGTH} caractères) : toutes les requêtes sont refusées.`);
    }
