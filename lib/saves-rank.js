...
// La transcription n'est pas lue ici (trop lourde pour une liste) : elle sert à trouver les candidats
// (colonne `search`, poids C) ; l'ordre s'appuie sur résumé, à retenir, recommandations et légende.
function textScore(r, words) {
  const meta = norm([r.author, r.category, r.kind, (r.tags ?? []).join(' '), r.cuisine, r.adresse, r.resume, r.retenir, r.repond_a, (r.recos ?? []).join(' ')].join(' '));
  const caption = norm(`${r.caption ?? ''} ${r.attention ?? ''}`);
  let score = 0;
  for (const w of words) {
    const rx = new RegExp(`\\b${w}`, 'g');
    score += 3 * count(meta, rx) + count(caption, rx);
  }
  return score;
}
...
const frDate = (d) => (d ? d.split('-').reverse().join('/') : null);

// Formule prête à citer : « d'après la vidéo de @compte enregistrée le 07/09/2026 (lien) ».
function citation(r) {
  const noun = r.media_type === 'post' ? 'la publication' : 'la vidéo';
  const date = frDate(r.saved_at);
  return `d'après ${noun} de @${r.author ?? '?'} ${date ? `enregistrée le ${date}` : "(date d'enregistrement inconnue)"} (${r.url})`;
}

// Réponse de la vidéo : question à laquelle elle répond, recommandations concrètes, réserves.
function insights(r, { maxRecos, recoLength }) {
  const lines = [];
  if (r.repond_a) lines.push(`Répond à : ${cut(r.repond_a, 200)}`);
  const recos = (r.recos ?? []).slice(0, maxRecos);
  if (recos.length) {
    lines.push(`Recommandations (${recos.length}/${r.recos.length}) :`, ...recos.map((x) => `- ${cut(x, recoLength)}`));
  }
  if (r.attention) lines.push(`Attention : ${cut(r.attention, 300)}`);
  return lines;
}
...
export function formatResults({ hits, need, weak }, plan) {
  ...
  hits.forEach((h, i) => {
    ...
    lines.push(...insights(h, { maxRecos: 4, recoLength: 200 }).map((l) => `   ${l}`));
    lines.push(`   Source à citer : ${citation(h)}`);
  });
  return lines.join('\n');
}

export function formatSave(r) {
  const lines = [header(r), UNTRUSTED, `Source à citer : ${citation(r)}`];
  const where = place(r);
  if (where) lines.push(`Lieu : ${where}`);
  ...
  lines.push(...insights(r, { maxRecos: 15, recoLength: 400 }));
